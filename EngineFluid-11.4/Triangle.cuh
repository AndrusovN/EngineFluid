...
private:
	...
	__host__ __device__ bool isInsideTriangle(Vector3 point) const;
public:
	__host__ __device__ Triangle(Vector3 A, Vector3 B, Vector3 C);
	__host__ __device__ Triangle();

	__host__ __device__ const Triangle operator = (const Triangle& other);
	...
	__host__ __device__ bool isInside(const Vector3& other) const;
